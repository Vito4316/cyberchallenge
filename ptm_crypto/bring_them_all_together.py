...
def mix_single_column(a):
    # The mix_column function mixes the elements of a single column
    t = a[0] ^ a[1] ^ a[2] ^ a[3]
    u = a[0]
    a[0] ^= t ^ xtime(a[0] ^ a[1])
    a[1] ^= t ^ xtime(a[1] ^ a[2])
...
def bytes2matrix(b):
    return [list(b[i*4:(i+1)*4]) for i in range(4)]
...
def decrypt(key, ciphertext):
    ...
    # Make sure round_keys[N_ROUNDS] is a 4x4 matrix
    state = add_round_key(state, round_keys[N_ROUNDS])

    for i in range(N_ROUNDS-1, 0, -1):
        inv_shift_rows(state)
        # sub_bytes returns a flat list, but we need a 4x4 matrix
        state = bytes2matrix(bytes(sub_bytes(state)))
